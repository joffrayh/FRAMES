...
def load_and_prepare_data(config, is_tabular=True):
    ...
    if not os.path.exists(path_info):
        path_info = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "..", "..", path_info)
        )

    df = pd.read_parquet(path_info, columns=config["data"]["columns"])
    ...
    if (
        ...
        if feat_cfg.get("use_rolling", False):
            ...
            df = _apply_rolling(
                feat_cfg, df, base_features, rolling_windows, timestep_hours
            )
    ...
    for w in range(target_window):
        targets.append(
            df.groupby("stay_id")["sepsis"].shift(-(target_horizon + w))
        )
