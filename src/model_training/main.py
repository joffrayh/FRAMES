...
from src.model_training.data.loader import (
    grouped_stratified_split,
    load_and_prepare_data,
)
from src.model_training.models.factory import get_model
from src.model_training.utils.metrics import (
    evaluate_model,
...
def load_config(args_config) -> dict:
    ...
    config_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), args_config)
    )
...
def log_environment():
    """Log hardware and software environment to MLflow for reproducibility."""
    mlflow.set_tags(
        {
            "sys.python_version": platform.python_version(),
            "sys.os": f"{platform.system()} {platform.release()}",
            "sys.cpu": platform.processor() or platform.machine(),
            "sys.cpu_cores_logical": str(os.cpu_count()),
            "sys.ram_gb": str(
    ...
    if lockfile.exists():
        mlflow.log_artifact(str(lockfile), artifact_path="environment")

    project_toml = Path(__file__).resolve().parents[2] / "pyproject.toml"
    if project_toml.exists():
        mlflow.log_artifact(str(project_toml), artifact_path="environment")


def main():
    """Run the model training framework end to end."""
    # loading the configs
    parser = argparse.ArgumentParser(
        description="Config-driven Sepsis Training Framework"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    if (
        ("tabular" in config)
        and ("models" in config["tabular"])
        and (active_model in config["tabular"]["models"])
    ):
        is_tabular = True
    ...
    with mlflow.start_run(
        run_name=f"{active_model.upper()}_Execution", log_system_metrics=True
    ):
        log_environment()
        ...
        if (
            is_tabular
            and ("tabular" in config)
            and ("feature_engineering" in config["tabular"])
        ):
        ...
        model_wrapper = get_model(
            active_model, config, model_cfg, train_features
        )
        ...
        metrics = evaluate_model(
            y_test, y_probs, f"{active_model.upper()} Test Performance"
        )
        mlflow.log_metrics({f"test_{k}": v for k, v in metrics.items()})

        calib_fig = plot_calibration_curve(
            y_test, y_probs, active_model.upper()
        )
        mlflow.log_figure(
            calib_fig, f"calibration_curve_{active_model.lower()}.png"
        )
        plt.close(calib_fig)
