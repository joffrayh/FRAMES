...
class LightGBMWrapper(BaseTabularModel):
    ...
    def fit_model(self, X_train, y_train, X_val, y_val):
        ...
        study = optuna.create_study(
            direction="maximize", study_name="sepsis-lightgbm-tuning"
        )
        study.optimize(
            objective,
            n_trials=opt_config.get("num_trials", 20),
    ...
    def custom_func(self, df_train, df_val, df_test, y_test, y_probs):
        shap_fig = shap_explanations(
            self.model, df_test[self.features], "LightGBM"
        )
