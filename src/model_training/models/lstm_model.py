...
class LSTMModelWrapper(BaseSequenceModel):
        ...
        for epoch in tqdm(range(epochs), desc="Epochs"):
            ...
            for X_b, y_b, mask_b in train_loader:
                ...
                nn.utils.clip_grad_norm_(
                    self.model.parameters(),
                    max_norm=float(self.lstm_params.get("clip_grad_norm", 1.0)),
                )
                optimizer.step()
            ...
            print(
                f"Epoch {epoch + 1:02d}/{epochs} | Train Loss: {train_loss:.4f} | Val AUPRC: {val_auprc:.4f}"
            )
    ...
    def save_model(self, model_name):
        ...
        input_schema = Schema(
            [TensorSpec(np.dtype(np.float32), (-1, -1, len(self.features)))]
        )
